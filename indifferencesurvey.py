...
def indifference_survey(maximum_value):
    ...
    if st.session_state["both_sneakers_image"] is not None and st.session_state["same_price_question"] is None:
        with st.form(key='buy ambush form at same price'):
            st.write(f"**Now please compare the following two options**: \n\nBoth the Air Force 1 and the Ambush are priced at **\${price}**")
            same_price = st.radio("Q1. Which option do you prefer?", (f"Option 1: Air Force 1 at a price of \${price}", f"Option 2: Ambush at a price of \${price}", "I am indifferent between Option 1 and Option 2"), index=None)
        ...
        with st.form(" Not Buy AirForce1 Price"):
            ...
            if "Option 1" in st.session_state["same_price_question"]:
                not_buy_ambush_price = st.number_input(
                    f"At equal prices (\${price}), you prefer the Air Force 1.\n\n **If the price of the Air Force 1 stayed at \${price}, how low would the price of the Ambush have to be for you to prefer it instead??**", \
            ...
            # with st.form(" Not Buy AirForce1Ambush Price"):
                not_buy_ambush_price = st.number_input(
                    f"At equal prices (\${price}), you prefer the Ambush. \n\n **If the price of the Air Force 1 stayed at \${price}, how high would the price of the Ambush have to be for you to prefer the Air Force 1 instead?**", \
    ...
    if st.session_state["not_buy_ambush_question"] is not None and st.session_state["verify_not_buy_ambush_question"] is None:
        with (st.form(" Verify Not Buy Ambush Price")):
            if "Option 1" in st.session_state["same_price_question"]:
                verify_not_buy_ambush = st.radio(f"Q3. Can you confirm that you would choose option 2 - the Ambush at \${st.session_state["not_buy_ambush_question"]} - over option 1, the Air Force 1 at \${price}?", ("Yes, I confirm that I would choose option 2", "No, I cannot confirm that I would choose option 2"), index=None)
            else:
                verify_not_buy_ambush = st.radio(f"Q3. Can you confirm that you would choose option 1 - the Air Force 1 at \${price} - over option 2, the Ambush at \${st.session_state["not_buy_ambush_question"]}?", ("Yes, I confirm that I would choose option 1", "No, I cannot confirm that I would choose option 1"), index=None)
            if st.form_submit_button("please confirm your choice"):
                if "Yes" in verify_not_buy_ambush:
                    ...
                    st.warning("You have not chosen a correct price for Ambush then. Please try again.")
    ...
    if st.session_state["verify_not_buy_ambush_question"] is not None and st.session_state["indifference_question"] is None:
        if "Option 1" in st.session_state["same_price_question"]:
            st.write(f"Based on the previous answers \n\n  - You chose the **Air Force 1** when the prices of both sneakers is \${price}\n\n  - You chose the **Ambush** when the price of the **Ambush is** \${st.session_state["not_buy_ambush_question"]} while the price of the **Air Force 1 is** \${price}")
            with st.form(" Indifference question"):
                indifference_number = st.number_input(f"Therefore, there is a price between \${st.session_state["not_buy_ambush_question"]} and \${price} for the Ambush where you are indifferent between the two sneakers. [i.e. you cannot choose between them]. Please fill out that price below \n\n", value=None, min_value=st.session_state["not_buy_ambush_question"], max_value=price, step=1)
                if st.form_submit_button("please confirm your choice"):
                    st.session_state["indifference_question"] = indifference_number
                    st.rerun()
        else:
            st.write(f"Based on the previous answers \n\n  - You chose the **Ambush** when the prices of both sneakers is \${price}\n\n  - You chose the **Air Force 1** when the price of the **Ambush is** \${st.session_state["not_buy_ambush_question"]} while the price of the **Air Force 1 is** \${price}")
            with st.form(" Indifference question"):
                indifference_number = st.number_input(f"Therefore, there is a price between \${price} and \${st.session_state["not_buy_ambush_question"]} for the Ambush where you are indifferent between the two sneakers. [i.e. you cannot choose between them]. Please fill out that price below \n\n", value=None, min_value=price, max_value=st.session_state["not_buy_ambush_question"], step=1)
                if st.form_submit_button("please confirm your choice"):
                    st.session_state["indifference_question"] = indifference_number
                    st.rerun()
        ...
        with st.form(" verify indifference question"):
            ...
            if st.form_submit_button("please confirm your choice"):
                if "Yes" in verify_indifferece:
                    ...
                    st.warning(f"‘Hmm....Since you are not indifferent between the two options, please input a price for Ambush at which you are truly indifferent between the two options.")
